...
const DetailsClient = () => {
  ...
  const deleteUser = () => {
    (async () => {
      ...
      toggleCanvas();
      response.status === 200
        ? setMessageSubmit("Usuário deletado com sucesso")
        : setMessageSubmit("Falha ao deletar usuário");
    })();
  };
  const toggleCanvas = () => {
  ...
  const ticketRequest = () => {
    (async () => {
      ...
      if (response.status === 200) {
        ...
        setDownloadPDF(
          "Falha ao gerar o carnê. Entre em contato com um administrador"
        );
      }
    })();
  };
    ...
    <div className="container-1">
        ...
        <div className="clients-btn-details-position">
          <div>
            ...
            <button
              ...
              style={{ backgroundColor: "white" }}
            >
              <DeleteOutlined onClick={toggleCanvas} /> Deletar
            </button>

            <button
              className="input-edit-outlined"
              onClick={ticketRequest}
              style={{ backgroundColor: "white" }}
            >
              Gerar carnê
            </button>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Tem certeza que deseja excluir o cliente?</Modal.Title>
        </Modal.Header>
        <Modal.Footer>
          <button className="input-edit-outlined" onClick={toggleCanvas}>
            Cancelar
          </button>
          <button className="input-delete-outlined" onClick={deleteUser}>
            Excluir
          </button>
        </Modal.Footer>
      </Modal>
      ...
      {messageSubmit && (
        <ModalMessage message={messageSubmit} navigateLink={"/clients"}/>
          ...
          Object.entries(clientData.lots).map(([key, value]) => (
            <div className="divide-details" key={key}>
              ...
              <div className="status-position">
                <p className="status-user-open">
                  <WarningOutlined />
                  ABERTO
                </p>
                <button
                  onClick={() => setToggleDeleteLot(!toggleDeleteLot)}
                  className="delete-user-allotements"
                >
                  Excluir lote do proprietário
                </button>
              </div>
              {toggleDeleteLot && (
                <div>
                  <h3>Tem certeza que deseja desvincular o lote?</h3>
                  <button onClick={() => setToggleDeleteLot(!toggleDeleteLot)}>
                    Cancelar
                  </button>
